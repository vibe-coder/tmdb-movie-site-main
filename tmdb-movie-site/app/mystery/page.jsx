...
import backdrop from "../images/Mystery/mystery.jpeg"
import Footer from '../Footer'

function page() {


  const [mysteryMovies, getMysteryMovies] = useState([])

  const fetchMysteryMovies = () => {
    const options = {
      method: 'GET',
      headers: {
    ...
    fetch('https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&language=en-US&page=1&sort_by=popularity.desc&with_genres=9648', options)
      ...
      .then(data => {
        ...
        console.log(data)
        const mysteryMoviesResult = data.results.slice(0, -2);

        console.log(mysteryMoviesResult)

        getMysteryMovies(mysteryMoviesResult)
    ...
    <>
      <div className='genre-container'>
        ...
        <div className='genre-movies-wrapper'>
          ...
          {
            mysteryMovies.map((mysteryMovieData) => {
              return(
                <div className='genre-movie-box' key={mysteryMovieData.id}>
                  <Image priority={true} className='genre-poster' src={imgPref + mysteryMovieData.poster_path} alt={mysteryMovieData.original_title} fill={true} sizes={100} />        
                </div>
              )
            })
          }
